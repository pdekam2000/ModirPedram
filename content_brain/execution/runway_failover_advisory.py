"""
Phase 11E-f — Runway failover readiness advisory (planning metadata only).

Produces advisory metadata for Runway session outcomes. Does not execute failover,
...
ADVISORY_VERSION = "11e_f_v1"
TIMESTAMP_FORMAT = "%Y-%m-%d %H:%M:%S"

RUNWAY_PROVIDER_IDS = frozenset({"runway", "runway_browser"})
...
def is_runway_provider(provider_id: str | None) -> bool:
    normalized = normalize_provider_id(str(provider_id or ""))
    if normalized == "runway_api":
        normalized = "runway"
    return normalized in RUNWAY_PROVIDER_IDS or normalized.startswith("runway")
...
def _extract_partial_artifacts(execution_runtime: dict[str, Any]) -> tuple[list[str], int]:
    ...
    if isinstance(artifacts, list):
        for artifact in artifacts:
            ...
            clip_result = _dict(meta.get("runway_clip_result"))
...
def _next_chain_candidate(plan, current_provider: str) -> tuple[str | None, str | None]:
    current = normalize_provider_id(current_provider)
    if current == "runway_api":
        current = "runway"
    seen_current = False
    for step in plan.chain:
        if step.provider_id == current:
            seen_current = True
            continue
        if seen_current and not step.blocked:
            return step.provider_id, None
    for step in plan.chain:
        if step.provider_id != current and not step.blocked:
            return step.provider_id, None
    blocked = [step for step in plan.chain if step.blocked and step.provider_id != current]
    if blocked:
...
def build_runway_failover_advisory(
    *,
    ...
    if provider == "runway_api":
        provider = "runway"
    if not is_runway_provider(provider):
        return None
    ...
    plan = failover_planner.plan_failover(
        ...
        constraints=FailoverConstraints(excluded_providers=(provider,)),
    )
