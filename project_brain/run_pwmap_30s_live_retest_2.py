"""Execute PWMAP 30s two-clip live retest #2 with safety + forensic capture.

Re-run of PHASE PWMAP-30S-TWO-CLIP-LIVE-RETEST after the pwmap-layer
download_selection.py repair, to confirm clip 1 and clip 2 are no longer
byte-identical. See project_brain/run_pwmap_30s_live_retest.py for retest #1
(FAIL — duplicate stale download reproduced, guards blocked correctly).
"""
...
def main() -> int:
    ...
    print("PWMAP 30s live retest #2 — preflight")
    ...
    if run_dir.is_dir():
        ...
        forensic["clip_1_size_bytes"] = clip1.stat().st_size if clip1.is_file() else 0
        forensic["clip_2_size_bytes"] = clip2.stat().st_size if clip2.is_file() else 0
        ...
        forensic["publish_ready"] = result.get("publish_ready") or result.get("publish_package_ready")
        forensic["assembly_status"] = result.get("assembly_status")
        forensic["branding_status"] = result.get("branding_status")
        forensic["final_publish_video_path"] = result.get("final_publish_video_path")
        forensic["youtube_upload_status"] = result.get("youtube_upload_status")
        forensic["youtube_upload_blocked_reason"] = result.get("youtube_upload_blocked_reason")
        forensic["youtube_visibility"] = result.get("youtube_visibility")
        forensic["youtube_url"] = result.get("youtube_url")
        forensic["visual_diversity_status"] = result.get("visual_diversity_status")
        forensic["repeated_clip_warning"] = result.get("repeated_clip_warning")
        forensic["stop_stage"] = result.get("stop_stage")
