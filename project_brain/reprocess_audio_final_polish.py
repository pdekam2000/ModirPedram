"""Re-run audio polish pipeline on existing run — no Runway, no clip regen."""
...
from content_brain.audio.audio_mastering_engine import (
...
AUDIO_FIXED_NAME = "FINAL_BRANDED_VIDEO_CANONICAL_AUDIO_FIXED.mp4"
PRIOR_FIXED = "FINAL_BRANDED_VIDEO_CANONICAL_FIXED.mp4"
SPEECH_WINDOW = 18.5


def _levels(path: Path) -> dict[str, float | None]:
    if not path.is_file():
        return {"full_mean_db": None, "speech_mean_db": None, "speech_max_db": None}
    return {
        "full_mean_db": probe_mean_volume_db(path),
        "speech_mean_db": probe_mean_volume_db(path, start_seconds=0, duration_seconds=SPEECH_WINDOW),
        "speech_max_db": probe_max_volume_db(path, start_seconds=0, duration_seconds=SPEECH_WINDOW),
    }
...
def reprocess_audio() -> dict[str, Any]:
    run_dir = RUN_DIR.resolve()
    profile = ProductChannelProfileStore(ROOT).load()
    prior = run_dir / "publish" / PRIOR_FIXED
    before = {
        "prior_fixed": _levels(prior),
        "narration_source": _levels(run_dir / "publish" / "narration" / "narration.mp3"),
    }
    ...
    audio_post_result = {
        ...
        "character_voice_status": "Character voices: reused narration (no new TTS)",
        "metadata": {"merge": merge_result.to_dict(), "env_mix": env_mix, "mastering": mastering, "music": music_result},
    }
    ...
    branding = run_branding_runtime(
        ...
        branded_video_name=AUDIO_FIXED_NAME,
    )
    ...
    audio_fixed_publish = run_dir / "publish" / AUDIO_FIXED_NAME
    if branded_source.is_file() and branded_source.resolve() != audio_fixed_publish.resolve():
        shutil.copy2(branded_source, audio_fixed_publish)
    publish_manifest["branded_video_path"] = str(audio_fixed_publish.resolve())
    ...
    final_path = audio_fixed_publish if audio_fixed_publish.is_file() else branded_source
    after = {
        "audio_fixed": _levels(final_path),
        "mastered_pre_brand": _levels(deliverable),
        "narrated_post_merge": _levels(Path(merge_result.output_path)),
        "normalized_narration": _levels(run_dir / "final" / "narration_normalized.mp3"),
    }
    package = load_story_package(ROOT, RUN_ID)
    names = [str(item.get("name") or "") for item in (package.get("character_profiles") or []) if isinstance(item, dict)]
    ...
    if visible is None and final_path.is_file():
        visible = measure_subtitle_text_bbox(final_path, 2.0).get("visible")
